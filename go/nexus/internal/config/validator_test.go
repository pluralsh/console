...
func TestValidateRateLimiting_NegativeRequests(t *testing.T) {
	cfg := config.Defaults()
	cfg.Console.GRPCEndpoint = "console:9090"
	cfg.RateLimiting.PerUser.RequestsPerMinute = -1 // Negative

	err := config.Validate(cfg)
	if err == nil {
		t.Error("expected error for negative requests per minute")
	}
	...
	found := false
	for _, e := range verr {
		if strings.Contains(e.Field, "perUser.requestsPerMinute") {
			found = true
		}
	}
	if !found {
...
func TestValidateRateLimiting_Disabled(t *testing.T) {
	cfg := config.Defaults()
	cfg.Console.GRPCEndpoint = "console:9090"
	...
	err := config.Validate(cfg)
	if err != nil {
		t.Errorf("expected no errors when rate limiting disabled, got: %v", err)
	}
}

func TestValidateObservability_InvalidLogLevel(t *testing.T) {
	cfg := config.Defaults()
	cfg.Console.GRPCEndpoint = "console:9090"
	cfg.Observability.LogLevel = "invalid" // Not debug, info, warn, or error
	...
	found := false
	for _, e := range verr {
		if e.Field == "observability.logLevel" {
			found = true
		}
	}
	if !found {
...
func TestValidateObservability_MetricsPathInvalid(t *testing.T) {
	cfg := config.Defaults()
	cfg.Console.GRPCEndpoint = "console:9090"
	cfg.Observability.Metrics.Path = "metrics" // Missing leading slash

	err := config.Validate(cfg)
	if err == nil {
		t.Error("expected error for invalid metrics path")
	}
	...
	found := false
	for _, e := range verr {
		if e.Field == "observability.metrics.path" && strings.Contains(e.Message, "/") {
			found = true
		}
	}
	if !found {
		t.Error("expected error about metrics path starting with /")
	}
}

func TestValidateObservability_TracingMissingConfig(t *testing.T) {
	cfg := config.Defaults()
	cfg.Console.GRPCEndpoint = "console:9090"
	cfg.Observability.Tracing.Enabled = true
	cfg.Observability.Tracing.Service = ""          // Missing
	cfg.Observability.Tracing.DatadogAgentHost = "" // Missing

	err := config.Validate(cfg)
	if err == nil {
		t.Error("expected at least 2 errors for tracing config")
	}
	...
	if len(verr) < 2 {
		t.Errorf("expected at least 2 errors for tracing config, got %d", len(verr))
	}

	serviceError := false
	agentError := false
	for _, e := range verr {
		if e.Field == "observability.tracing.service" {
			serviceError = true
		}
		if e.Field == "observability.tracing.datadogAgentHost" {
			agentError = true
		}
	}
	...
	if !agentError {
		t.Error("expected error for missing datadog agent host")
	}
}
