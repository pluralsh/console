...
func NewBedrockRouter(client *bifrostcore.Bifrost, resolver *EmbeddingResolver) Router {
	return (&BedrockRouter{
		GenericRouter: &GenericRouter{client: client, resolver: resolver},
	}).init()
}

func (in *BedrockRouter) init() Router {
	in.logger = log.Logger().With(zap.String("component", "bifrost-bedrock-router"))
...
func (in *BedrockRouter) newEmbeddingsRoute() RouteConfig {
	return RouteConfig{
		...
		RequestConverter: func(ctx *schemas.BifrostContext, req interface{}) (*schemas.BifrostRequest, error) {
			...
			if err := in.resolver.Apply(schemas.Bedrock, bifrostReq); err != nil {
				return nil, err
			}
