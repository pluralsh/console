...
var _ = Describe("Security Context", func() {
	for _, chartEntry := range console.Charts() {
		Context(chartEntry.Name+" with custom console security context values", Ordered, func() {
			var (
		...
		Context(chartEntry.Name+" with custom console pod security context values", Ordered, func() {
			var (
				deployment      appsv1.Deployment
				err             error
				manifests       common.ManifestMap
