...
const (
	...
	MonitorReconciler                    Reconciler = "monitor"
	DashboardReconciler                  Reconciler = "dashboard"
)
...
func (rl ReconcilerList) ToControllers(mgr ctrl.Manager, url, token string, datadogEnabled, insecureSkipTLSVerify bool,
	...
	for i, r := range rl {
		controller, err := r.ToController(mgr, client.New(url, token, datadogEnabled, insecureSkipTLSVerify), credentialsCache)
		if err != nil {
			return nil, nil, err
		}
