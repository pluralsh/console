...
// +kubebuilder:validation:XValidation:rule="!has(self.graphs) || self.graphs.all(g, !has(g.sectionId) || size(g.sectionId) == 0 || self.graphs.exists(s, s.type == 'SECTION' && s.identifier == g.sectionId))",message="sectionId must reference an existing SECTION graph"
...
// +kubebuilder:validation:XValidation:rule="self.type != 'SECTION' || !has(self.sectionId) || size(self.sectionId) == 0",message="sections cannot be nested, SECTION graphs cannot set sectionId"
