...
func TestGetServiceDetail(t *testing.T) {
	...
	}{
		{
			...
			expected: &ServiceDetail{
				Service: Service{
					...
					InternalEndpoint:  common.Endpoint{Host: "svc-1.ns-1", Ports: make([]common.ServicePort, 0)},
		...
		},
		{
			...
			expected: &ServiceDetail{
				Service: Service{
					...
					Selector:          map[string]string{"app": "app2"},
					TypeMeta:          types.TypeMeta{Kind: types.ResourceKindService},
					InternalEndpoint:  common.Endpoint{Host: "svc-2.ns-2", Ports: make([]common.ServicePort, 0)},
					ExternalEndpoints: []common.Endpoint{},
				},
	...
	for _, c := range cases {
		fakeClient := fake.NewClientset(c.service)
