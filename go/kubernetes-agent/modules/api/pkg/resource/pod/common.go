...
func getPodStatus(pod v1.Pod) string {
	...
	if !initializing || isPodInitializedConditionTrue(&pod.Status) {
		...
		for i := len(pod.Status.ContainerStatuses) - 1; i >= 0; i-- {
			...
			case container.State.Terminated != nil && len(container.State.Terminated.Reason) > 0:
				reason = container.State.Terminated.Reason
			case container.State.Terminated != nil && len(container.State.Terminated.Reason) == 0 && container.State.Terminated.Signal != 0:
...
func getPodConditions(pod v1.Pod) []common.Condition {
	conditions := make([]common.Condition, 0, len(pod.Status.Conditions))
	for _, condition := range pod.Status.Conditions {
