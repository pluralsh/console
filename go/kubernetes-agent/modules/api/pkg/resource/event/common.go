...
func GetNodeEvents(client kubernetes.Interface, dsQuery *dataselect.DataSelectQuery, nodeName string) (*common.EventList, error) {
	...
	eventsInvUID, err := client.CoreV1().Events(v1.NamespaceAll).SearchWithContext(context.Background(), scheme, node)
	...
	eventsInvName, err := client.CoreV1().Events(v1.NamespaceAll).SearchWithContext(context.Background(), scheme, node)
	_, criticalError = errors.ExtractErrors(err)
	if criticalError != nil {
		return &eventList, criticalError
	}
