...
| `repositoryImage` _string_ | RepositoryImage is an OCI image of precloned git repositories plus manifest.json.<br />When set, agent-run pods mount it read-only at /plural/repos via a Kubernetes<br />image volume so bootstrap can copy a matching repo locally instead of git clone. |  | Optional: \{\} <br /> |
