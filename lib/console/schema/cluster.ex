defmodule Console.Schema.Cluster do
  ...
  def allow_mgmt(query, false), do: from(c in query, where: not c.self or is_nil(c.self))
  def allow_mgmt(query, _), do: query
  ...
  def ai_enabled(query \\ __MODULE__) do
    from(c in query,
      ...
      where: is_nil(c.disable_ai) or not c.disable_ai
    )
  end
