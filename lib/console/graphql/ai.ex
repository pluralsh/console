defmodule Console.GraphQl.AI do
  ...
  object :ai_subscriptions do
    ...
    field :ai_stream, :string do
      ...
      config fn
        %{insight_id: id}, %{context: %{current_user: user}} -> {:ok, topic: Stream.topic(:inssight, id, user)}
        %{thread_id: id}, %{context: %{current_user: user}} -> {:ok, topic: Stream.topic(:thread, id, user)}
