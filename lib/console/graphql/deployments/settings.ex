defmodule Console.GraphQl.Deployments.Settings do
  ...
  input_object :deployment_settings_attributes do
    ...
    field :prometheus_connection, :http_connection_attributes,
      description: "connection details for a prometheus instance to use"

    field :loki_connection, :http_connection_attributes,
      description: "connection details for a loki instance to use"

    field :logging, :logging_settings_attributes
    field :mgmt_repo, :string

    field :smtp, :smtp_settings_attributes,
      description: "configuration for smtp message delivery"

    field :ai, :ai_settings_attributes, description: "configuration for LLM provider clients"

    field :cost, :cost_settings_attributes,
      description: "settings for cost management functionality"

    field :metrics, :metrics_settings_attributes,
      description: "settings for OpenTelemetry metrics export"
  ...
  input_object :metrics_settings_attributes do
    ...
    field :crontab, :string,
      description: "cron expression for how often to export metrics (e.g. '*/5 * * * *')"
  end
  ...
  input_object :cost_settings_attributes do
    ...
    field :recommendation_threshold, :integer,
      description: "the percentage change needed to generate a recommendation, default 30%"

    field :recommendation_cushion, :integer,
      description: "the percentage change needed to generate a recommendation, default 20%"
  end
  ...
  input_object :opensearch_connection_attributes do
    ...
    field :use_pod_identity, :boolean,
      description:
  ...
  input_object :openai_settings_attributes do
    ...
    field :proxy_models, list_of(:string),
      description: "addditional models to support within the integrated ai proxy"

    field :token_exchange, :openai_token_exchange_attributes,
      description: "OAuth2 client credentials against a token endpoint to obtain access tokens"

    field :headers, list_of(:openai_header_attributes),
      description: "custom HTTP headers to include in OpenAI-compatible API requests"
  end

  input_object :openai_header_attributes do
    field :name,  non_null(:string)
  ...
  input_object :ollama_attributes do
    ...
    field :authorization, :string,
      description: "An http authorization header to use on calls to the Ollama api"
  ...
  input_object :bedrock_ai_attributes do
    ...
    field :aws_secret_access_key, :string,
      description: "the aws secret access key to use (DEPRECATED)"
  ...
  input_object :vertex_ai_attributes do
    ...
    field :service_account_json, :string,
      description: "optional service account json to auth to the GCP vertex apis"

    field :endpoint, :string,
      description: "custom vertexai endpoint if for dedicated customer deployments"
  ...
  input_object :aws_cloud_connection_attributes do
    ...
    field :assume_role_arn, :string,
      description: "optional IAM role ARN for the console to assume when using this connection"
  end
  ...
  object :project do
    ...
    field :read_bindings, list_of(:policy_binding),
      ...
      description: "read policy across this project"

    field :write_bindings, list_of(:policy_binding),
      resolve: dataloader(Deployments),
      description: "write policy across this project"
  ...
  object :deployment_settings do
    ...
    field :enabled, non_null(:boolean),
      description: "whether you've yet to enable CD for this instance"

    field :name, non_null(:string)

    field :self_managed, :boolean,
      description: "whether the byok cluster has been brought under self-management"

    field :loki_connection, :http_connection,
      description: "the way we can connect to your loki instance"

    field :prometheus_connection, :http_connection,
      description: "the way we can connect to your prometheus instance"
    ...
    field :logging, :logging_settings,
    ...
    field :agent_vsn, non_null(:string),
      description: "The console's expected agent version",
      resolve: fn _, _, _ -> {:ok, Settings.agent_vsn()} end

    field :latest_k8s_vsn, non_null(:string),
      description: "the latest known k8s version",
      resolve: fn _, _, _ -> {:ok, Settings.kube_vsn()} end

    field :compliant_k8s_vsn, non_null(:string),
      description: "your compliant k8s version",
      resolve: fn _, _, _ -> {:ok, Settings.compliant_vsn()} end

    field :artifact_repository, :git_repository,
      resolve: dataloader(Deployments),
      description: "the repo to fetch CAPI manifests from, for both providers and clusters"

    field :deployer_repository, :git_repository,
      resolve: dataloader(Deployments),
      description: "the repo to fetch the deploy operators manifests from"

    field :read_bindings, list_of(:policy_binding),
      resolve: dataloader(Deployments),
      description: "read policy across all objects"

    field :write_bindings, list_of(:policy_binding),
      resolve: dataloader(Deployments),
      description: "write policy across all objects"

    field :git_bindings, list_of(:policy_binding),
      resolve: dataloader(Deployments),
      description: "policy for managing git repos"

    field :create_bindings, list_of(:policy_binding),
      resolve: dataloader(Deployments),
      description: "policy for creation of new objects"
  ...
  object :ai_settings do
    ...
    field :tools_enabled, :boolean,
      resolve: fn _, _, _ -> {:ok, Console.AI.Provider.tools?()} end
  ...
  object :openai_settings do
    ...
    field :token_exchange, :openai_token_exchange,
      description: "OAuth2 client credentials configured for token endpoint exchange"

    field :headers, list_of(:openai_header),
      description: "custom HTTP headers included in OpenAI-compatible API requests"
  end

  object :openai_header do
    field :name,  non_null(:string)
    field :value, non_null(:string)
  end
  ...
  object :bedrock_ai_settings do
    ...
    field :access_key_id, :string,
      description: "the openai bedrock aws access key id to use (DEPRECATED)",
  ...
  object :logging_settings do
    ...
    field :driver, :log_driver,
      description: "the type of log aggregation solution you wish to use"

    field :victoria, :http_connection, description: "configures a connection to victoria metrics"

    field :elastic, :elasticsearch_connection,
      description: "configures a connection to elasticsearch for logging"
  ...
  object :opensearch_connection do
    ...
    field :use_pod_identity, :boolean,
      description: "whether to use pod identity (IRSA/Workload Identity) for AWS authentication"
  end
  ...
  object :cloud_connection do
    ...
    field :configuration, non_null(:cloud_connection_configuration),
      description: "the configuration for the cloud connection"

    field :read_bindings, list_of(:policy_binding),
      resolve: dataloader(Deployments),
      description: "read policy across this cloud connection"
  ...
  object :aws_connection_attributes do
    ...
    field :assume_role_arn, :string,
      description: "IAM role ARN for the console to assume when using this connection"
  end
