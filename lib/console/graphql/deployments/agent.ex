defmodule Console.GraphQl.Deployments.Agent do
  ...
  alias Console.Schema.{AgentRuntime, AgentRun, AgentRunUpload}
  ...
  input_object :agent_runtime_attributes do
    ...
    field :browser_enabled,      :boolean, description: "whether this runtime provisions a headless browser sidecar (and the browser-use MCP tools) for runs"
  ...
  input_object :agent_run_upload_attributes do
    field :session,          :upload, description: "the agent session recording"
    field :screen_recording, :upload, description: "a screen recording of the agent run"
    field :patch,            :upload, description: "a patch file generated by the agent run"
  end
  ...
  object :agent_run_upload do
    field :id, non_null(:id)

    field :session, :string, description: "url for the agent session recording",
      resolve: &upload_url/3
    field :screen_recording, :string, description: "url for the screen recording",
      resolve: &upload_url/3
    field :patch, :string, description: "url for the patch file",
      resolve: &upload_url/3

    field :agent_run, :agent_run, resolve: dataloader(Deployments),
      description: "the agent run these uploads belong to"
  ...
  object :public_agent_mutations do
    ...
    field :create_agent_run_upload, :agent_run_upload do
      middleware ClusterAuthenticated
      arg :run_id, non_null(:id)
      arg :attributes, non_null(:agent_run_upload_attributes)

      resolve &Deployments.create_agent_run_upload/2
    end
  end
