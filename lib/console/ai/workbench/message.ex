defmodule Console.AI.Workbench.Message do
  ...
  def to_message(%WorkbenchJobActivity{tool_call: %{call_id: id, name: name, arguments: arguments}} = activity),
    do: {:tool, String.trim(message_prompt(activity: activity)), %{call_id: id, name: name, arguments: arguments}}
  def to_message(%WorkbenchJobActivity{} = activity), do: {:assistant, String.trim(message_prompt(activity: activity))}
  def to_message(pass), do: pass
