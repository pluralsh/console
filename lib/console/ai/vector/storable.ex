...
defimpl Console.AI.Vector.Storable, for: Console.Schema.Cluster.Mini do
  ...
  def content(%@for{} = mini) do
    base_content(mini)
    |> append_yaml("runtime_services", mini.runtime_services)
    |> append_yaml("cloud_addons", mini.cloud_addons)
    |> append_yaml("upgrade_plan", mini.upgrade_plan)
    |> append_yaml("metadata", mini.metadata)
  end

  defp append_yaml(base, field_name, content) do
    case TU.yaml_encode(content) do
      {:ok, yaml} -> "#{base}\n#{field_name}:\n#{TU.indent(yaml, 2)}"
      _ -> "\n#{field_name}: #{inspect(content)}"
    end
  end
...
defimpl Console.AI.Vector.Storable, for: Console.Schema.WorkbenchJob.Mini do
  ...
  def content(%@for{} = mini), do: content_template(job: mini)

  EEx.function_from_file(:defp, :content_template, Console.priv_filename(["prompts", "vector", "workbench_job.md.eex"]), [:assigns], trim: true)
end
