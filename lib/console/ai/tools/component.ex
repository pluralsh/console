defmodule Console.AI.Tools.Component do
  ...
  def implement(%__MODULE__{service_deployment: service, cluster: cluster} = comp) do
    with {:flow, %Flow{id: flow_id}} <- {:flow, Console.AI.Tool.flow()},
      ...
      {:comp, err} ->
        map = Map.from_struct(comp)
              |> Map.take(~w(api_version kind namespace name)a)
        {:ok, "#{Poison.encode!(map)} is likely not a member of the Plural service #{service}, full error: #{inspect(err)}"}
