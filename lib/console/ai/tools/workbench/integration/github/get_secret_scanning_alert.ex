defmodule Console.AI.Tools.Workbench.Integration.Github.GetSecretScanningAlert do
  @moduledoc false
  ...
  @json_schema Console.priv_file!("tools/workbench/integration/github/get_secret_scanning_alert.json")
               |> Jason.decode!()

  def name(%__MODULE__{tool: %WorkbenchTool{name: n}}),
    do: "github_#{n}_get_secret_scanning_alert"

  def description(%__MODULE__{tool: %WorkbenchTool{name: n}}),
    do: "Get one secret scanning alert for a repository (#{n}) via GitHub REST."
  ...
  def implement(
    ...
    with {:ok, client} <- Client.build(m.tool) do
      client
      |> Client.json_get("repos/#{m.owner}/#{m.repo}/secret-scanning/alerts/#{m.alert_number}")
      |> Response.json()
    end
  end
