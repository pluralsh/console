defmodule Console.AI.Tools.Workbench.Integration.Github.GetTag do
  @moduledoc false
  ...
  @json_schema Console.priv_file!("tools/workbench/integration/github/get_tag.json") |> Jason.decode!()

  def name(%__MODULE__{tool: %WorkbenchTool{name: n}}), do: "github_#{n}_get_tag"

  def description(%__MODULE__{tool: %WorkbenchTool{name: n}}),
    do: "Get a git tag ref (#{n}) via GitHub REST."
  ...
  def implement(%__MODULE__{tool: %WorkbenchTool{configuration: %Configuration{github: %GithubConnection{}}}} = m) do
    with {:ok, client} <- Client.build(m.tool) do
      get("repos/#{m.owner}/#{m.repo}/git/refs/tags/#{URI.encode(m.tag)}", client)
      |> Response.json()
    end
  end
