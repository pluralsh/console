defmodule Console.AI.Tools.Workbench.Integration.Github.ListBranches do
  @moduledoc false
  ...
  @json_schema Console.priv_file!("tools/workbench/integration/github/list_branches.json") |> Jason.decode!()

  def name(%__MODULE__{tool: %WorkbenchTool{name: n}}), do: "github_#{n}_list_branches"

  def description(%__MODULE__{tool: %WorkbenchTool{name: n}}),
    do: "List branches (#{n}) via GitHub REST."

  def json_schema(%__MODULE__{}), do: @json_schema

  def changeset(m, attrs), do: m |> cast(attrs, [:owner, :repo, :page, :per_page]) |> validate_required([:owner, :repo])
  ...
  def implement(%__MODULE__{tool: %WorkbenchTool{configuration: %Configuration{github: %GithubConnection{}}}} = m) do
    with {:ok, client} <- Client.build(m.tool) do
      ...
      |> then(&get("repos/#{m.owner}/#{m.repo}/branches#{Query.qp(&1)}", client))
      |> Response.json()
    end
  end
