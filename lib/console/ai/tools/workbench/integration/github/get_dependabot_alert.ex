defmodule Console.AI.Tools.Workbench.Integration.Github.GetDependabotAlert do
  @moduledoc false
  ...
  @json_schema Console.priv_file!("tools/workbench/integration/github/get_dependabot_alert.json")
               |> Jason.decode!()

  def name(%__MODULE__{tool: %WorkbenchTool{name: n}}),
    do: "github_#{n}_get_dependabot_alert"

  def description(%__MODULE__{tool: %WorkbenchTool{name: n}}),
    do: "Get one dependency vulnerability alert from Dependabot for a repository (#{n}) via GitHub REST."
  ...
  def implement(
    ...
    with {:ok, client} <- Client.build(m.tool) do
      client
      |> Client.json_get("repos/#{m.owner}/#{m.repo}/dependabot/alerts/#{m.alert_number}")
      |> Response.json()
    end
  end
