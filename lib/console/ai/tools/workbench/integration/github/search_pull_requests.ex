defmodule Console.AI.Tools.Workbench.Integration.Github.SearchPullRequests do
  @moduledoc false
  ...
  def description(%__MODULE__{tool: %WorkbenchTool{name: n}}),
    do: "Search pull requests (#{n}) via GitHub REST search API."
  ...
  defp scoped_query(%{query: query, owner: o, repo: r}) when is_binary(o) and is_binary(r),
    do: query <> " repo:#{o}/#{r} is:pr"

  defp scoped_query(%{query: query}), do: query <> " is:pr"
end
