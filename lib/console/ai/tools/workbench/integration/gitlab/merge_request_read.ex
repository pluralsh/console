defmodule Console.AI.Tools.Workbench.Integration.Gitlab.MergeRequestRead do
  @moduledoc false
  ...
  @json_schema Console.priv_file!("tools/workbench/integration/gitlab/merge_request_read.json") |> Jason.decode!()

  def name(%__MODULE__{tool: %WorkbenchTool{name: n}}), do: "gitlab_#{n}_merge_request_read"

  def description(%__MODULE__{tool: %WorkbenchTool{name: n}}),
    do:
      "Fetch a GitLab merge request by project and IID (#{n}); optionally include notes (comments) with pagination."

  def json_schema(%__MODULE__{}), do: @json_schema

  def changeset(m, attrs) do
    m
    |> cast(attrs, [:project, :merge_request_iid, :include_notes, :page, :per_page])
  ...
  def implement(%__MODULE__{tool: %WorkbenchTool{configuration: %Configuration{gitlab: %GitlabConnection{}}}} = m) do
    with {:ok, client} <- Client.build(m.tool),
         ...
         {:ok, mr} <- Client.get(client, "/projects/#{pid}/merge_requests/#{m.merge_request_iid}") do
      if m.include_notes do
        ...
        with {:ok, notes} <- Client.get(client, "/projects/#{pid}/merge_requests/#{m.merge_request_iid}/notes", query) do
          Jason.encode(%{"merge_request" => mr, "notes" => notes})
        end
      else
        Jason.encode(mr)
      end
