defmodule Console.Deployments.Pr.Dispatcher do
  ...
  def create(%PrAutomation{} = pr, branch, ctx) when is_binary(branch) do
    ...
    with {:ok, conn} <- setup(%{conn | branch: template_branch(pr.branch, ctx)}, pr.identifier, branch),
         ...
         {:ok, pr} <- ai_edit(pr, conn.dir, ctx),
         {:ok, msg} <- maybe_render_message(pr, ctx),
  ...
  def pr_data(%ScmWebhook{} = hook, body) do
  ...
  defp maybe_render_message(%PrAutomation{ignore_templates: true, message: msg}, _),
    do: {:ok, msg}
  defp maybe_render_message(%PrAutomation{} = pr, ctx),
    do: render_solid(pr.message, ctx)
