defmodule Console.Deployments.Policy do
  ...
  defp stabilize_vulns(%{vulnerabilities: [_ | _] = vulns} = attrs, current_vulns) do
    ...
    vulns = Enum.sort_by(vulns, & {&1[:resource], &1[:target], &1[:pkg_path], &1[:installed_version], &1[:primary_link], &1[:score], &1[:published_date]})
  ...
  defp vuln_changeset(%Ecto.Changeset{changes: %{vulnerabilities: [_ | _] = vulns}} = changeset) do
    count =
      Enum.filter(vulns, & map_size(&1.changes) > 0)
      |> Enum.map(&IO.inspect(&1.changes, label: "vuln changeset for #{&1.data.resource} #{&1.data.target} #{&1.data.pkg_path} #{&1.data.installed_version} #{&1.data.primary_link} #{changeset.data.artifact_url}"))
      |> Enum.count()
    Logger.info("number of updated vulns: #{count} for #{changeset.data.artifact_url}")
    changeset
  end
  defp vuln_changeset(changeset), do: changeset
end
