...
defmodule Console.Deployments.Compatibilities.AddOn do
  import  Console.Deployments.Compatibilities.Utils, only: [versions_greater: 2]
  ...
  def upgrade_version(addon, cluster, from \\ nil)
  def upgrade_version(%__MODULE__{versions: [_ | _] = vsns}, %Cluster{current_version: cv}, from) when is_binary(cv) do
    cleaned = clean_version(cv)
    with {:ok, %Version{major: maj, minor: min}} <- Version.parse(cleaned) do
      Enum.reverse(vsns)
      |> versions_greater(from)
      |> Enum.find(fn
  ...
  def upgrade_version(_, _, _), do: nil
