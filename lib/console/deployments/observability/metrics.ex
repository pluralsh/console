defmodule Console.Deployments.Observability.Metrics do
  ...
  @cluster post_process([
    ...
    cpu_usage: 'sum(rate (container_cpu_usage_seconds_total{container!="",cluster="$cluster"}[5m]))',
    memory_usage: 'sum(container_memory_working_set_bytes{image!="",cluster="$cluster",container!=""})'
  ])

  @node post_process([
    cpu: 'sum (rate (container_cpu_usage_seconds_total{container!="",cluster="$cluster",node="$instance"}[5m])) / sum (machine_cpu_cores{node="$instance",cluster="$cluster"})',
    ...
    cpu_usage: 'sum(rate(container_cpu_usage_seconds_total{container!="",node="$instance",cluster="$cluster"}[5m]))',
    memory_usage: 'sum(container_memory_working_set_bytes{image!="",container!="",node="$instance"})'
  ])

  @component post_process([
    cpu: 'sum(rate(container_cpu_usage_seconds_total{container!="",cluster="$cluster",namespace="$namespace",pod=~"$name$regex"}[5m]))',
    mem: 'sum(container_memory_working_set_bytes{cluster="$cluster",namespace="$namespace",pod=~"$name$regex",image!="",container!=""})',
    pod_cpu: 'sum(rate(container_cpu_usage_seconds_total{container!="",cluster="$cluster",namespace="$namespace",pod=~"$name$regex"}[5m])) by (pod)',
