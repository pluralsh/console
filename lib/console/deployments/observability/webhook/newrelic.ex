defmodule Console.Deployments.Observability.Webhook.Newrelic do
  ...
  def associations(:project, payload, acc),
    do: Map.put(acc, :project_id, extract_from_payload(payload, ~r/plrl_project:\s*([^,\s]+)/, &project/1))
  def associations(:cluster, payload, acc),
    do: Map.put(acc, :cluster_id, extract_from_payload(payload, ~r/plrl_cluster:\s*([^,\s]+)/, &cluster/1))
