defmodule Console.OCI.Auth do
  ...
  def authenticate(client, prov, %OCIAuth{proxy: proxy} = auth) do
    do_authenticate(Client.with_proxy(client, proxy), prov, auth)
  end

  def authenticate(client, prov, auth), do: do_authenticate(client, prov, auth)

  defp do_authenticate(client, prov, auth) when prov in ~w(aws gcp azure)a,
    do: sidecar_auth(client, prov, auth || %{})
  defp do_authenticate(client, :basic, %OCIAuth{basic: %{username: u, password: p}}),
    do: {:ok, Client.with_credentials(client, u, p)}
  defp do_authenticate(client, :bearer, %OCIAuth{bearer: %{token: token}}) when is_binary(token),
    do: {:ok, Client.with_token(client, token)}
  defp do_authenticate(client, _, _), do: {:ok, client}
