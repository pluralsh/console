defmodule Console.Commands.Plural do
  ...
  def template(conf, dir, ext, env \\ [])
  def template(conf, dir, nil, env), do: cmd_tee("plural", ["pr", "template", "--file", conf], dir, env)
  def template(conf, dir, ext, env) when is_binary(ext), do: cmd_tee("plural", ["pr", "template", "--file", conf, "--templates", ext], dir, env)
