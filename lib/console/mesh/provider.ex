defmodule Console.Mesh.Provider do
  ...
  alias Console.Mesh.Provider.{Istio, Ebpf, Linkerd, Cilium}
  ...
  defp client(
    %Cluster{operational_layout: %OperationalLayout{service_mesh: :cilium}} = cluster,
    %DeploymentSettings{prometheus_connection: %Connection{host: h} = conn}
  ) when is_binary(h), do: {:ok, Cilium.new(conn, cluster)}
