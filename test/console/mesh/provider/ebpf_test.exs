defmodule Console.Mesh.Provider.EbpfTest do
  ...
  describe "#graph/2" do
    test "it can fetch the graph for a cluster" do
      cluster = insert(:cluster, operational_layout: %{service_mesh: :ebpf})
  ...
  defp metric(source, destination) do
    %{
      ...
      "source.container.name" => "nginx-container",
      "dest.workload.name" => "nginx",
      "dest.namespace.name" => destination,
      "dest.container.name" => "nginx-container"
    }
  end
end
