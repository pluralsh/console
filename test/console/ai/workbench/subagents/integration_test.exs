defmodule Console.AI.Workbench.Subagents.IntegrationTest do
  ...
  describe "new/1" do
    ...
    @tag :skip
    test "it can handle slack tools in the agent loop" do
      deployment_settings(
        logging: %{enabled: true, driver: :elastic, elastic: es_settings()},
        ai: %{
          enabled: true,
      ...
      expect(Provider, :completion, fn _, _ ->
        {:ok, "list slack channels",
         [
           %Tool{
             name: "slack_list_channels_slack",
             arguments: %{},
             id: "1"
           }
         ]}
      ...
      tool =
        insert(:workbench_tool,
          tool: :slack,
          name: "slack",
          configuration: %{slack: %{bot_token: System.get_env("SLACK_BOT_TOKEN")}}
        )
