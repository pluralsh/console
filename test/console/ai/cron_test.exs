defmodule Console.AI.CronTest do
  ...
  describe "#services/0" do
    ...
    test "it will gather info from unknown custom resources and generate" do
      ...
      expect(Kube.Utils, :run, fn _ -> {:ok, %{items: []}} end)
      expect(Kube.Utils, :run, fn _ -> {:ok, stateful_set("name", "ns", 1, [])} end)
      expect(Kube.Utils, :run, fn _ -> {:ok, %{items: []}} end)
      expect(Kube.Utils, :run, fn _ -> {:ok, %{items: []}} end)
    ...
    test "it will gather evidence from a statefulset and its pvcs" do
      ...
      # Create cluster with operational_layout for namespace resolution
      ...
      expect(Console.Logs.Provider, :query, fn %Console.Logs.Query{
        query: "ebs.csi.aws.com",
      ...
      assert evidence_text =~ "here are the logs for the csi provisioner ebs.csi.aws.com in the aws-ebs-csi-driver namespace"
