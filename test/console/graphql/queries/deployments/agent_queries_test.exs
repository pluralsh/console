defmodule Console.GraphQL.Queries.Deployments.AgentQueriesTest do
  ...
  describe "agentRuntime" do
    ...
    test "a user can fetch a runtime by name" do
      user = insert(:user)
      runtime = insert(:agent_runtime, name: "gemini", create_bindings: [%{user_id: user.id}])
      insert(:agent_runtime, name: "claude")

      {:ok, %{data: %{"agentRuntime" => found}}} = run_query("""
        query AgentRuntime($name: String!) {
          agentRuntime(name: $name) {
            id
            name
          }
        }
      """, %{"name" => "gemini"}, %{current_user: Console.Services.Rbac.preload(user)})

      assert found["id"] == runtime.id
      assert found["name"] == "gemini"
    end

    test "a user cannot fetch a runtime by name they cannot create runs on" do
      user = insert(:user)
      _runtime = insert(:agent_runtime, name: "gemini")
      insert(:agent_runtime, name: "claude", create_bindings: [%{user_id: user.id}])

      {:ok, %{errors: [_ | _]}} = run_query("""
        query AgentRuntime($name: String!) {
          agentRuntime(name: $name) { id }
        }
      """, %{"name" => "gemini"}, %{current_user: Console.Services.Rbac.preload(user)})
    end

    test "a cluster can fetch a runtime by name" do
      cluster = insert(:cluster)
      runtime = insert(:agent_runtime, name: "gemini", cluster: cluster)
      _other = insert(:agent_runtime, name: "claude", cluster: cluster)
      ...
      """, %{"name" => "gemini"}, %{cluster: cluster})

      assert found["id"] == runtime.id
      assert found["name"] == "gemini"
    end

    test "a cluster cannot fetch a runtime by name from another cluster" do
      cluster = insert(:cluster)
      other_cluster = insert(:cluster)
      _runtime = insert(:agent_runtime, name: "gemini", cluster: other_cluster)
    ...
    test "returns error when neither name nor id is specified" do
      user = insert(:user)
      ...
      assert error["message"] == "Must specify either name or id"
    end
  end
