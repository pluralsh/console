...
func (r *PipelineReconciler) Reconcile(ctx context.Context, req ctrl.Request) (_ reconcile.Result, reterr error) {
	...
	utils.SyncNamespacedCredentialsAnnotation(pipeline, nc)
	utils.MarkCredentialsCondition(pipeline.SetCondition, nc, err)
	if err != nil {
...
func (r *PipelineReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(&v1alpha1.NamespaceCredentials{}, utils.HandleCredentialsChange(r.Client, new(v1alpha1.PipelineList))). // Reconcile objects if namespaced credentials change.
