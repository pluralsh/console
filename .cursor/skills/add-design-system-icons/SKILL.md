...
description: Adds new icon components to the Console design system and exports them from `design-system/src/icons.ts`. Use when creating a new icon from a pasted SVG, converting a Figma-exported SVG into a `createIcon` component, or adding a new brand/logo icon with optional `fullColor` support.
---
...
Target files:
- `design-system/src/components/icons/<IconName>.tsx`
- `design-system/src/icons.ts`

## Quick Rules

- `mix format` SHOULD NEVER BE RUN in this repository.
- Follow the existing icon pattern in `design-system/src/components/icons/createIcon.tsx`.
...
- Add the export to `design-system/src/icons.ts` in alphabetical order.
...
- Create `design-system/src/components/icons/<IconName>.tsx`.
...
- Create `design-system/src/components/icons/<BrandName>LogoIcon.tsx` unless the user requested a different name.
...
- Add `export { default as <IconName> } from './components/icons/<IconName>'` to `design-system/src/icons.ts`
...
**Brand logo request**
- "Add logo icons for Elasticsearch, Loki, Prometheus, and Tempo to `design-system/src/icons.ts` with full-color support."
