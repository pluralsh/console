...
export const render = async (
      ...
      nodeArray.map(async function (node: {
        ...
        if (node) {
          ...
          nodeDb[node.id].offset = {
            ...
            width: Math.max(
              node.width,
              node.labels ? node.labels[0]?.width || 0 : 0
            ),
            height: node.height,
          }
          if (node.isGroup) {
            ...
            clusterNode.width = Math.max(
              clusterNode.width,
              node.labelData.width
            )
            ...
            log.info(
              ...
              relY,
              node.domId.node(),
              `translate(${node.x + relX + node.width / 2}, ${
                node.y + relY + node.height / 2
              })`
            )
            node.domId.attr(
              'transform',
  ...
  const addSubGraphs = (nodeArr: any[]): TreeData => {
    ...
    subgraphs.forEach((subgraph: { id: string }) => {
      const children = nodeArr.filter(
        (node: { parentId: any }) => node.parentId === subgraph.id
      )
  ...
  const getCandidateBorderPoint = (
    ...
    if (side === 'start') {
      ...
      const candidate =
        centerApprox && points.length > 1 ? points[points.length - 2] : last
      return { candidate, centerApprox }
    }
  }
  ...
  const applyStartIntersectionIfNeeded = (
    points: P[],
    startNode: any,
    ...
    if (firstOutsideStartIndex !== -1) {
      ...
      log.debug('UIO cutter2: start-only intersection applied', {
        startIntersection,
      })
    }
  }

  const applyEndIntersectionIfNeeded = (
    points: P[],
    endNode: any,
    endBounds: RectLike
  ) => {
    ...
    if (outsideIndexForEnd !== -1) {
      ...
      log.debug('UIO cutter2: end-only intersection applied', {
        endIntersection,
      })
    }
  }
  ...
  const cutter2 = (startNode: any, endNode: any, _points: any[]) => {
    ...
    if (outsidePointForEnd) {
      ...
      log.debug('UIO cutter2: end intersection', {
        endIntersection,
        outsideIndexForEnd,
      })
      replaceEndpoint(points, 'end', endIntersection)
  ...
  let elkGraph: any = {
    ...
    layoutOptions: {
      ...
      'elk.layered.considerModelOrder.strategy':
        data4Layout.config.elk?.considerModelOrder,
      'elk.layered.unnecessaryBendpoints': true,
      'elk.layered.cycleBreaking.strategy':
        data4Layout.config.elk?.cycleBreakingStrategy,
  ...
  elkGraph.edges.forEach((edge: any, index: number) => {
    ...
    if (
      nodeDb[source] &&
      nodeDb[target] &&
      nodeDb[source].parentId !== nodeDb[target].parentId
    ) {
  ...
  elkGraph.edges?.forEach((edge: any, index: number) => {
    ...
    if (edge.sources) {
      edge.sources.forEach((sourceId: any) => {
        const sourceExists = elkGraph.children?.some(
          (child: any) => child.id === sourceId
        )
    ...
    if (edge.targets) {
      edge.targets.forEach((targetId: any) => {
        const targetExists = elkGraph.children?.some(
  ...
  try {
    ...
    log.error(
      'APA01 elkGraph that caused error:',
    ...
    (edge: {
      ...
      if (edge.sections) {
        ...
        if (startNode.isGroup) {
          ...
          sw = Math.max(
            startNode.width,
            startNode.labels[0].width + startNode.padding
          )
          // sw = startNode.width;
          log.info(
            'UIO width',
        ...
        if (endNode.isGroup) {
          ...
          ew = Math.max(
            endNode.width,
            endNode.labels[0].width + endNode.padding
          )
        ...
        // Block for reducing variable scope and guardrails for the cutter function
        {
          ...
          const skipStart =
            startIsGroup && onBorder(startBounds, startCandidate)
          const skipEnd = endIsGroup && onBorder(endBounds, endCandidate)

          dropAutoCenterPoint(
            prevPoints as P[],
            'start',
            skipStart && startCenterApprox
          )
          dropAutoCenterPoint(
            prevPoints as P[],
            'end',
            skipEnd && endCenterApprox
          )
          ...
          if (skipStart || skipEnd) {
            ...
            log.debug(
              'PPP cutter2: skipping cutter2 due to on-border group endpoint(s)',
              {
                skipStart,
                skipEnd,
                startCenterApprox,
        ...
        log.debug(
          'PPP cutter2: Points after cutter2:',
          JSON.stringify(edge.points)
        )
        ...
        if (
          ...
          const cleaned = prevPoints.filter(
        ...
        const deduped = edge.points.filter(
          (
            ...
            const prev = arr[i - 1]
            return (
              Math.abs(p.x - prev.x) > 1e-6 || Math.abs(p.y - prev.y) > 1e-6
            )
          }
        )
