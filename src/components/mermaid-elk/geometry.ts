...
/* Geometry utilities for ELK layout edge routing */
  ...
  if (Math.abs(y - outsidePoint.y) * w > Math.abs(x - outsidePoint.x) * h) {
    ...
    let _x =
      insidePoint.x < outsidePoint.x
        ...
        : insidePoint.x - R + r
    let _y =
      insidePoint.y < outsidePoint.y ? insidePoint.y + q : insidePoint.y - q
...
export const ensureTrulyOutside = (
  bounds: RectLike,
  p: P,
  push = PUSH_OUT
): P => {
...
export const tryNodeIntersect = (
  ...
  const wrongSide =
    (outside.x < bounds.x && res.x > bounds.x) ||
    (outside.x > bounds.x && res.x < bounds.x)
  if (wrongSide) {
    return null
  }
...
export const fallbackIntersection = (
  bounds: RectLike,
  outside: P,
  center: P
): P => {
...
export const computeNodeIntersection = (
  ...
  return (
    tryNodeIntersect(node, bounds, outside2) ??
    fallbackIntersection(bounds, outside2, center)
  )
}
