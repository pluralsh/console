...
app_name = "psmdb-operator"
compatibility_url = (
    "https://raw.githubusercontent.com/percona/k8spsmdb-docs/main/docs/versions.md"
)
...
def _extract_version(cell: str) -> str:
    ...
    if not match:
        raise ValueError(f"Unsupported PSMDB Operator version cell: {cell!r}")
...
def parse_platform_matrix(
    ...
    for line in lines[2:]:
        ...
        if version in seen:
            raise ValueError(f"Duplicate PSMDB Operator row: {version}")
        seen.add(version)
    ...
    if not rows:
        raise ValueError("No PSMDB Operator releases matched official Helm charts")
    return rows
...
def scrape() -> None:
    ...
    if not page:
        raise ValueError("Could not fetch Percona PSMDB compatibility matrix")
