...
app_name = "pg-operator"
compatibility_url = (
    "https://raw.githubusercontent.com/percona/k8spg-docs/3.0/docs/versions.md"
)
...
def _extract_version(cell: str) -> str:
    ...
    if not match:
        raise ValueError(f"Unsupported PostgreSQL Operator version cell: {cell!r}")
...
def _platform_table(markdown: str) -> list[str]:
    lines = markdown.splitlines()
    heading_index = None
    for index, line in enumerate(lines):
        if re.fullmatch(r"\s*(?:#{1,6}\s*)?Platforms:?\s*", line, re.IGNORECASE):
            heading_index = index
            break
...
def parse_platform_matrix(
    ...
    lines = _platform_table(markdown)
    ...
    for line in lines[2:]:
        ...
        if version in seen:
            raise ValueError(f"Duplicate PostgreSQL Operator row: {version}")
        seen.add(version)
    ...
    if not rows:
        raise ValueError("No PostgreSQL Operator releases matched official Helm charts")
    return rows
...
def scrape() -> None:
    ...
    if not page:
        raise ValueError("Could not fetch Percona PostgreSQL compatibility matrix")
