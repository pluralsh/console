...
def scrape():
    ...
    elastic_releases = list(reversed(list(get_github_releases_timestamps("elastic", app_name))))
    chart_versions = get_chart_versions(app_name)
    versions = []

    pruned_releases = [(r.lstrip("v"), ts) for r, ts in elastic_releases if "-" not in r]
    for idx, elastic_release in enumerate(pruned_releases):
        release_vsn = elastic_release[0]
        future_release = elastic_release
