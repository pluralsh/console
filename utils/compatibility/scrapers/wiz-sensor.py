...
def fetch_helm_index():
    ...
    if response.status_code != 200:
        print_error(f"Failed to fetch Helm index. Status code: {response.status_code}")
        return None
    return yaml.safe_load(response.text)


def get_chart_entries():
    """Get wiz-sensor chart entries from the Helm index."""
...
def scrape():
    ...
    if not chart_entries:
        print_error("No chart entries found for wiz-sensor.")
        return
    ...
    for entry in chart_entries:
        ...
        print_success(f"Fetched compatibility info for wiz-sensor {app_version} (chart {chart_version})")
