...
def helm_summary(name, compatibility, from_vsn, to_vsn):
  chart_url = compatibility.get('chart_changelog_url')
  if not chart_url:
      return None
  ...
  with open(os.path.join(os.path.dirname(__file__), "tools/version_summary.json")) as f:
    ...
    urls = set(chart_url.replace('{chart_version}', vsn) for vsn in [from_vsn['chart_version'], to_vsn['chart_version']])
    ...
    response = exa.get_content(list(release_urls), text=True)
    release_pages = [result.text for result in response.results]
    ...
    response = oai_client.responses.create(
        ...
        input="Here are the helm changelogs for the versions of {name} from {from_vsn['version']} to {to_vsn['version']}:\n\n" + "\n\n".join(pages) + "\n\nAnd here are the applicationrelease notes for the versions of {name} from {from_vsn['version']} to {to_vsn['version']}:\n\n" + "\n\n".join(release_pages),
