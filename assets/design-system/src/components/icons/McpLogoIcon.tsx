...
/** MCP logomark; geometry from official brand assets (MIT): https://github.com/modelcontextprotocol/modelcontextprotocol/tree/main/docs/logo */
const MCP_MARK_PATHS = [
...
export default createIcon(({ size, color, fullColor, mode }) => {
  const stroke = fullColor ? (mode === 'dark' ? '#ffffff' : '#000000') : color

  return (
    <svg
      width={size}
      height={size}
      viewBox="20 20 165 168"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      {MCP_MARK_PATHS.map((d) => (
        <path
          key={d}
          d={d}
          stroke={stroke}
