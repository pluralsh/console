...
export const callout: BaseSchema = {
  render: Callout,
  ...
  transform(node, config) {
    ...
    const finalChildren = children.filter(
      (child) => isTag(child) && child?.name !== 'Cta'
    )
    const ctas = children
      .filter((child): child is Tag => isTag(child) && child?.name === 'Cta')
      .map((cta) => ({
