...
import { parse, TestSuites, TestSuite, TestCase } from './junitParse'
...
describe('junitParse', () => {
  describe('parse', () => {
    ...
    it('should return null for invalid XML', () => {
      expect(parse('<not-junit><invalid></not-junit>')).toBeNull()
    })

    it('should return null for XML without testsuite or testsuites', () => {
      expect(parse('<root><something /></root>')).toBeNull()
    })
    ...
    describe('testsuites parsing', () => {
      it('should parse a simple testsuites element', () => {
        ...
        const result = parse(xml) as TestSuites

        expect(result).not.toBeNull()
        expect(result.name).toBe('All Tests')
    ...
    describe('testsuite parsing', () => {
      it('should parse a standalone testsuite element', () => {
        const xml = `
        ...
        const result = parse(xml) as TestSuite

        expect(result).not.toBeNull()
        expect(result.name).toBe('MySuite')
        expect(result.tests).toBe(2)
        expect(result.failures).toBe(0)
        expect(result.errors).toBe(0)
        expect(result.time).toBe(0.123)
        expect(result.testcase).toHaveLength(2)
      })
      ...
      it('should parse testsuite with all attributes', () => {
        ...
        expect(result.name).toBe('FullSuite')
        ...
        expect(result.timestamp).toBe('2024-01-15T12:00:00')
        expect(result.hostname).toBe('localhost')
        expect(result.id).toBe('suite-1')
        expect(result.package).toBe('com.example.tests')
      })
    })
    ...
    describe('testcase parsing', () => {
      it('should parse testcase with all attributes', () => {
        ...
        const result = parse(xml) as TestSuite
        const testcases = getTestCases(result)
    ...
    describe('system-out and system-err parsing', () => {
      ...
      it('should parse system-out in testsuite', () => {
        ...
        expect(result['system-out']).toEqual(['Suite level output'])
      })
    ...
    describe('properties parsing', () => {
      it('should parse properties in testsuite', () => {
        ...
        expect(result.properties![0].name).toBe('os')
        expect(result.properties![0].value).toBe('linux')
        expect(result.properties![1].name).toBe('java.version')
        expect(result.properties![1].value).toBe(11)
      })
      ...
      it('should parse single property without array wrapper', () => {
        ...
        expect(result.properties![0].name).toBe('single')
    ...
    describe('custom fastXmlOptions', () => {
      it('should respect custom options', () => {
        ...
        const result = parse(xml, { trimValues: false }) as TestSuite
    ...
    describe('real-world examples', () => {
      it('should parse a typical Jest JUnit output', () => {
        ...
        expect(result.name).toBe('jest tests')
        expect(result.tests).toBe(3)
        expect(result.failures).toBe(1)
