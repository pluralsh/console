...
export function Icon({tooltip, icon, iconProps, onClick, background, hoverIndicator}) {
  ...
  return (
    ...
    {hover && (
      <Drop target={ref.current} align={{bottom: 'top'}} plain>
        <Box round='xsmall' pad={{vertical: 'xsmall', horizontal: 'small'}} background='sidebar'>
...
function UpgradePolicy({policy}) {
  ...
  return (
    <Box direction='row' align='center' border={{side: 'bottom', color: 'tone-light'}}
         pad={{vertical: 'xsmall'}}>
      <Box fill='horizontal' direction='row' gap='small' align='center'>
        <Text size='small' weight={500}>{policy.name}</Text>
        <Text size='small'><i>{policy.description}</i></Text>
      </Box>
      <Icon icon={Trash} iconProps={{size: '14px', color: 'red'}} tooltip='delete' onClick={mutation} />
    </Box>
  )
}
  ...
  return (
    <Box gap='xsmall'>
      {upgradePolicies.map((policy) => (
        <UpgradePolicy key={policy.id} policy={policy} />
      ))}
      <Box pad={{top: 'small'}} direction='row' justify='end'>
        <Button label='Create More' onClick={() => setModal({
    ...
    <PolicyContext.Provider value={{modal, setModal, close}}>
      ...
      {modal && (
        <Layer modal onEsc={close} onClickOutside={close}>
          <Box width='50vw'>
            <ModalHeader text={modal.header} setOpen={setModal} />
            <Box fill pad='small'>
              {modal.content}
            </Box>
          </Box>
