...
export function Installer({setOpen}) {
  ...
  return (
    <Layer modal onEsc={close} onClickOutside={close}>
      {v === Views.CONF && <Configuration recipe={recipe} setOpen={setOpen} />}
      {v === Views.REC && <Recipes repo={repo} setOpen={setOpen} setRecipe={setRecipe} />}
      {v === Views.SEARCH && <SearchRepos setOpen={setOpen} setRepo={setRepo} />}
    </Layer>
  )
}