...
function Reaction({name, reactions, me, messageId}) {
  ...
  return (
    <Tooltip>
      <Box {...BOX_ATTRS} background='highlight' gap='xsmall'
           onClick={() => mutation({variables: {name: name}})}>
        <Text size='10px'>
          <Emoji set='google' emoji={name} size={18} style={{lineHeight: 0}} />
        </Text>
    ...
    <Box direction='row' margin={{top: 'xsmall'}} align='center'>
      <Box direction='row' gap='xsmall' height='25px' margin={{right: 'xsmall'}} align='center'>
        {sorted.map(([name, reactions]) => (
          <Reaction key={name} me={me} name={name} reactions={reactions} messageId={message.id} />
        ))}
      </Box>
      <Box flex={false} direction='row' height='25px' className='message-reactions' align='center'>
        <MessageReaction message={message} setHover={setHover} align={{bottom: 'top'}} 
          label='+' direction='row' gap='2px' background='white' border={{color: 'light-6'}} round='xsmall'
          {...BOX_ATTRS} />
      </Box>
    </Box>
  )
