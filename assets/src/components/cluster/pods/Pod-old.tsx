...
export function Pod() {
  ...
  const { pod } = data
  const containerStatuses = (pod.status.containerStatuses || []).reduce((acc, container) => ({ ...acc, [container.name]: container }),
    {})
  const initContainerStatuses = (pod.status.initContainerStatuses || []).reduce((acc, container) => ({ ...acc, [container.name]: container }),
    {})
        ...
        <Tabs defaultTab="info">
            ...
            {initContainers.map(({ name }) => (
              ...
              >
                ...
                >
                  <ReadyIcon
                    size="12px"
                    readiness={containerStatusToReadiness(initContainerStatuses[name])}
                  />
                  <Text
                    size="small"
                    weight={500}
          ...
          {initContainers.map(container => (
            ...
            >
              <Container
                container={container}
                containerStatus={initContainerStatuses[container.name]}
              />
            </TabContent>
          ))}
